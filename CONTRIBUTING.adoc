...
Want to contribute to a JBoss Logging project? We try to make it easy, and all contributions, even the smaller ones,
...
All original contributions to JBoss Logging projects are licensed under the
...
The DCO text is also included verbatim in the link:./dco.txt[dco.txt] file in the root directory of the repository.
...
JBoss Logging projects have a strictly enforced code style. Code formatting is done by the Eclipse code formatter, using the config files
found in the https://raw.githubusercontent.com/jboss-logging/logging-dev-tools/main/ide-config/src/main/resources/eclipse-code-formatter.xml[eclipse-code-formatter.xml]
...
Next navigate to _Java_ -> _Code Style_ -> _Organize Imports_. Click _Import_ and select the https://raw.githubusercontent.com/jboss-logging/logging-dev-tools/main/ide-config/src/main/resources/eclipse.importorder[eclipse.importorder] file.
...
By submitting a pull request, you agree to the https://developercertificate.org/[Developer Certificate of Origin (DCO)]
...
* Open a discussion in the repository's Discussions section
